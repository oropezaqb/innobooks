...
@section('content')
    <div class="col-md-12">
        <div class="card">
            <div class="card-header font-weight-bold">Company: {{ \Auth::user()->currentCompany->company->name }} (Sales Receipt Details)</div>
            <div class="card-body">
                <div id="wrapper">
                    <div id="page" class="container">
                        <div class="content">
                            ...
                            @endif
                            <form method="POST" action="/sales_receipts/{{ $salesReceipt->id }}">
                                @csrf
                                @method('PUT')
                                @if ($errors->any())
                                ...
                                <div class="form-group custom-control-inline">
                                    ...
                                    <input list="customer_ids" id="customer_id0" onchange="setValue(this)" data-id="" class="custom-select @error('customer_id') is-danger @enderror" required value="{!! old('customer_name', $salesReceipt->customer->name) !!}">
                                    <datalist id="customer_ids">
                                        @foreach ($customers as $customer)
                                            <option data-value="{{ $customer->id }}">{{ $customer->name }}</option>
                                        @endforeach
                                    </datalist>
                                    <input type="hidden" name="customer_id" id="customer_id0-hidden" value="{!! old('customer_id', $salesReceipt->customer_id) !!}">
                                    <input type="hidden" name="customer_name" id="name-customer_id0-hidden" value="{!! old('customer_name', $salesReceipt->customer->name) !!}">
                                </div>
                                <br><br>
                                <div class="form-group custom-control-inline">
                                    <label for="date">Sales&nbsp;Receipt&nbsp;date&nbsp;</label>&nbsp;
                                    <input type="date" class="form-control @error('date') is-danger @enderror" id="date" name="date" required value="{!! old('date', $salesReceipt->date) !!}">
                                </div>
                                <div class="form-group custom-control-inline">
                                    <label for="number">Sales&nbsp;Receipt&nbsp;no.&nbsp;</label>&nbsp;
                                    <input type="number" class="form-control" id="number" name="number" style="text-align: right;" required value="{!! old('number', $salesReceipt->number) !!}">
                                </div>
                                <br><br>
                                <div class="form-group custom-control-inline">
                                    <label for="account_id">Deposit&nbsp;to</label>&nbsp;
                                    <input list="account_ids" id="account_id0" onchange="setValue(this)" data-id="" class="custom-select @error('account_id') is-danger @enderror" required value="{!! old('account_name', $salesReceipt->account->title) !!}">
                            ...
                            <div style="float: right; clear: both;">
                                <div style="display: inline-block;">
                                    <button class="btn btn-primary" onclick="location.href = '/sales_receipts/{{ $salesReceipt->id }}/edit';">Edit</button>
                                </div>
                                <div style="display: inline-block;">
                                    <form method="POST" action="/sales_receipts/{{ $salesReceipt->id }}">
                                        @csrf
                            ...
                            <script>
                                ...
                                @if (!empty(old('item_lines')))
                                    ...
                                    updateTotal();
                                @else
                                    @if (!empty($salesReceipt->itemLines))
